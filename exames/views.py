...
def solicitar_exames(request):
    ...
    if request.method == "GET":
         # .objects acessa todos os dados da tabela
                                                 # .all() pega todos os dados da tabela
                                                 # .filter() você pode filtrar por nome, status, ou seja, pelas colunas

        # O context { } permite que a variável tipos_exames seja usada no html para retornar os dados
        ...
        #TODO: Calcular preco dos dados
        preco_total = 0
        for i in solicitacao_exames:
            if i.disponivel:
                preco_total += i.preco

        return render(request, 'solicitar_exames.html', {'tipos_exames': tipos_exames, # type: ignore
                                                         'solicitacao_exames': solicitacao_exames,
                                                         'preco_total':preco_total
                                                         })

@login_required
def fechar_pedidos(request):
    exames_id = request.POST.getlist('exames')
    solicitacao_exames = TiposExames.objects.filter(id__in=exames_id)
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        if not exame.resultado:
            messages.add_message(request,constants.ERROR,"O resultado ainda não foi cadastrado, entre em contato com o laboratório")
            redirect('/exames/solicitar_exames/')
    ...
    if request.method == "GET":
        ...
        if senha == exame.senha:
            if not exame.resultado:
                messages.add_message(request,constants.ERROR,"O resultado ainda não foi cadastrado, entre em contato com o laboratório")
                return redirect(f'/exames/solicitar_senha_exame/{exame_id}')
            elif senha == exame.senha and exame.resultado:
                return redirect(exame.resultado.url)
