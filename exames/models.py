...
class TiposExames(models.Model):
    ...
    nome = models.CharField(max_length=50)#CharField é um tipo que é obrigatório passar o max_length=
    tipo = models.CharField(max_length=1, choices=tipo_choices)
    preco = models.FloatField()
    disponivel = models.BooleanField(default=True) # Senão passar nada por padrão é True
    horario_inicial = models.IntegerField()
    horario_final = models.IntegerField()

    def __str__(self):
        # Mostra o nome da classe ao invês de nomes estranhos 
        return self.nome # Pesquisar no youtube sobre o método 
...
class SolicitacaoExame(models.Model):
    ...
    def badge_template(self):
        if self.status == 'E':
            ...
            texto = "Finalizado"
        return mark_safe(f"<span class='badge bg-primary {classes_css}'>{texto}</span>") #type: ignore
